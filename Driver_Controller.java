    ...
    public String getClient(String Username) {
        ...
        for (int i = 0; i < Trip.Ride.size(); i++) {
            if (Trip.Ride.get(i).equals(Username)) {
                return Trip.Ride.get(i + 1);
            }
        }
        return "Not Found";
    }

    public void setBalance(String dname, int b) {
        int Dindex = 0;
        for (int i = 0; i < DS.get_size(); i++) {
            if (DS.get(i).getName().equals(dname)) {
                Dindex = i;
        ...
        Driver_Storage.driverBalance.set(Dindex, b);
    }

    public int getBalance(String dname) {
        int Dindex = 0;
        for (int i = 0; i < DS.get_size(); i++) {
            if (DS.get(i).getName().equals(dname)) {
                Dindex = i;
        ...
        return Driver_Storage.driverBalance.get(Dindex);
    }
